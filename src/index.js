    ...
    function moveElement(element, targetElement, nameElement, classElement) {
        ...
        if ((new RegExp(targetElement.parentElement.parentElement.querySelector('.filter-frends__main-list-search').value, 'i'))
            .test(currentLi.querySelector('.filter-frends__item-name').textContent)) {
            let newLiElement = document.createElement(nameElement);
